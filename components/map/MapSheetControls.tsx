...
const MAP_CONTROLS_SCREEN_INSET = 16;
...
const MAP_DISPLAY_PILL_SCREEN_MARGIN = 12;
const MAP_DISPLAY_PILL_HORIZONTAL_PADDING = 10;
const MAP_DISPLAY_PILL_ICON_SIZE = 11;
const MAP_DISPLAY_PILL_ICON_GAP = 4;
const MAP_DISPLAY_PILL_LABEL_CHAR_WIDTH = 7.0;
...
type ScrubBackgroundSegment = ScrubChipPosition & { key: string; width: number; height: number };
...
function getScrubChipPosition(choice: BucketChoice, index = 0): ScrubChipPosition {
...
function getScrubBackgroundSegments(
  choices: readonly BucketChoice[],
  positions: readonly ScrubChipPosition[],
): ScrubBackgroundSegment[] {
  const firstChoice = choices[0];
  const panelInset = SCRUB_PANEL_PADDING;
  const segmentWidth = SCRUB_CHIP_SIZE + panelInset * 2;

  if (firstChoice?.type !== "horizon") {
    const bounds = getScrubClusterBounds(positions);
    return [
      {
        key: "vertical",
        x: -panelInset,
...
function getHorizonScrubBackgroundMetrics(
  choices: readonly BucketChoice[],
  positions: readonly ScrubChipPosition[],
) {
  const stackPositions = positions.filter((position) => position.x === SCRUB_COLUMN_X);
  const allPosition = positions[choices.findIndex((choice) => choice.key === "all")];

  if (!allPosition || stackPositions.length === 0) return null;

  const panelInset = SCRUB_PANEL_PADDING;
  const radius = 16;
  const stackLeft = SCRUB_COLUMN_X - panelInset;
  const stackTop = Math.min(...stackPositions.map((position) => position.y)) - panelInset;
  const stackRight = SCRUB_COLUMN_X + SCRUB_CHIP_SIZE + panelInset;
  const stackBottom =
    Math.max(...stackPositions.map((position) => position.y + SCRUB_CHIP_SIZE)) + panelInset;
  const allLeft = allPosition.x - panelInset;
  const allTop = allPosition.y - panelInset;
  const allBottom = allPosition.y + SCRUB_CHIP_SIZE + panelInset;

  const minX = allLeft;
  const minY = stackTop;
  const maxX = stackRight;
  const maxY = stackBottom;
  const strokeInset = 2;
  const width = maxX - minX + strokeInset * 2;
  const height = maxY - minY + strokeInset * 2;
  const viewBox = `${minX - strokeInset} ${minY - strokeInset} ${width} ${height}`;
  const path = [
    `M ${stackLeft + radius} ${stackTop}`,
    `H ${stackRight - radius}`,
    `Q ${stackRight} ${stackTop} ${stackRight} ${stackTop + radius}`,
    `V ${stackBottom - radius}`,
    `Q ${stackRight} ${stackBottom} ${stackRight - radius} ${stackBottom}`,
    `H ${stackLeft + radius}`,
    `Q ${stackLeft} ${stackBottom} ${stackLeft} ${stackBottom - radius}`,
    `V ${allBottom}`,
    `H ${allLeft + radius}`,
    `Q ${allLeft} ${allBottom} ${allLeft} ${allBottom - radius}`,
    `V ${allTop + radius}`,
    `Q ${allLeft} ${allTop} ${allLeft + radius} ${allTop}`,
    `H ${stackLeft}`,
    `V ${stackTop + radius}`,
    `Q ${stackLeft} ${stackTop} ${stackLeft + radius} ${stackTop}`,
    "Z",
  ].join(" ");

  return {
    left: minX - strokeInset,
    top: minY - strokeInset,
    width,
    height,
    viewBox,
    path,
...
function MapDisplayBucketPill({ label }: { label: string }) {
  ...
  const maxPillWidth = Math.max(MAP_BUTTON_SIZE, screenWidth - MAP_DISPLAY_PILL_SCREEN_MARGIN * 2);
  const estimatedPillWidth = Math.ceil(
    MAP_DISPLAY_PILL_HORIZONTAL_PADDING * 2 +
      MAP_DISPLAY_PILL_ICON_SIZE +
      MAP_DISPLAY_PILL_ICON_GAP +
      label.length * MAP_DISPLAY_PILL_LABEL_CHAR_WIDTH,
  );
  const pillWidth = Math.min(maxPillWidth, Math.max(MAP_BUTTON_SIZE, estimatedPillWidth));
  const centeredRight = (MAP_BUTTON_SIZE - pillWidth) / 2;
  const clampedRight =
    centeredRight <= -MAP_CONTROLS_SCREEN_INSET
      ? MAP_DISPLAY_PILL_SCREEN_MARGIN - MAP_CONTROLS_SCREEN_INSET
      : centeredRight;
  ...
  return (
    ...
    >
      <View
        className="flex-row items-center justify-center rounded-full px-2.5 py-0.5 bg-surface/95 border border-accent/40 shadow-sm"
        style={{ maxWidth: maxPillWidth }}
      >
...
function ZoomBucketScrubOverlay({
  ...
  const isHorizonScrub = bucketChoices[0]?.type === "horizon";
  const backgroundSegments = React.useMemo(
    () => getScrubBackgroundSegments(bucketChoices, positions),
    [bucketChoices, positions],
  );
  const horizonBackground = React.useMemo(
    () => (isHorizonScrub ? getHorizonScrubBackgroundMetrics(bucketChoices, positions) : null),
    [bucketChoices, isHorizonScrub, positions],
  );
  const colors = useThemeColors();
      ...
      >
        {horizonBackground && (
          <View
            ...
            style={{
              left: horizonBackground.left,
              top: horizonBackground.top,
              width: horizonBackground.width,
              height: horizonBackground.height,
            }}
          >
            <Svg
              width={horizonBackground.width}
              height={horizonBackground.height}
              viewBox={horizonBackground.viewBox}
            >
              <Path
                d={horizonBackground.path}
                fill={colors.surface}
        ...
        {bucketChoices.map((choice, index) => {
          ...
          const position = positions[index] ?? getScrubChipPosition(choice, index);
          return (
            <View
              key={choice.key}
              className="absolute"
